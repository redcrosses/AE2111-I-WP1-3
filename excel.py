...
def cruise_speed_list():
	...
	return c.cruise_massfraction/cruise_thrustlapse*((zerolift_drag*0.5*cruise_density*cruise_speed**2)/(c.cruise_massfraction*wing_loading)+(c.cruise_massfraction*wing_loading[10])/(math.pi*c.bypass_ratio*oswald_efficiency*0.5*cruise_speed**2*cruise_density))
...
class climb_gradient():
	...
	def climb_gradient_list(self):
		climbgradient_maxcl = math.sqrt(c.climbgradient_zerodrag[self.num]*math.pi*c.bypass_ratio)
		...
		oswald_efficiency = 1/(math.pi*c.bypass_ratio*c.parasite_drag+(1/c.span_efficiency))
		return c.climbgradient_massfraction[self.num]/climbgradient_thrust_lapse*(c.climbgradient_gradient[self.num]/100+2*math.sqrt(c.climbgradient_zerodrag[self.num]/(math.pi*c.climbgradient_oswaldfactor[self.num]*c.bypass_ratio)))


def to_field_length_list():
	to_pressure = 101325*(1+(-0.0065*c.to_altitude/288.15))**(-9.81/(-0.0065*287))
	to_density = to_pressure/(287*c.to_temp)
	to_velocity = np.sqrt((wing_loading*2)/(to_density*c.to_cL))
	to_mach_number = to_velocity/np.sqrt(1.4*287*288.15)
	to_total_pressure = (1+(1.4-1)/2*np.power(to_mach_number,2))*101325
